        ...
        <div className='bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200 rounded-2xl p-6 shadow-lg max-[515px]:p-2'>
          <div className='flex items-center justify-between'>
            <div className='flex items-center'>
              <div
                className={`bg-yellow-100 p-3 rounded-full ${language === 'ar' ? 'ml-4' : 'mr-4'}`}
              >
              ...
              <div>
                ...
                <div className='mb-4'>
                  ...
                  <div className='relative max-w-xs'>
                    ...
                    />
                    <span
                      className={`absolute ${language === 'ar' ? 'left-3' : 'right-3'} top-1/2 transform -translate-y-1/2 text-yellow-600 text-sm font-medium`}
                    >
                      {/* {language === 'ar' ? 'د.ج' : 'DZD'} */}
                    </span>
                  </div>
                ...
                {/* Nissab Display */}
                {goldPricePerGram &&
                  parseFloat(goldPricePerGram.replace(/,/g, '')) > 0 && (
                    <div className='bg-yellow-100/50 border border-yellow-300 rounded-lg p-4'>
                      <div className='space-y-2'>
                        <p className='text-yellow-800 text-sm'>
                          <span className='font-semibold'>
                            {t('goldPrice.pricePerGram')}:
                          </span>{' '}
                          {formatCurrency(
                            parseFloat(goldPricePerGram.replace(/,/g, ''))
                          )}
                        </p>
                        <p className='text-yellow-800 text-sm'>
                          <span className='font-semibold'>
                            {t('goldPrice.nissab')} (85 {t('goldPrice.grams')}):
                          </span>{' '}
                          {formatCurrency(nissab)}
                        </p>
                      </div>
                    </div>
                  )}
