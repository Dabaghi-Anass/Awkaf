...
export const CalForm = () => {
	...
	const calculateTotalForField = (fieldName, mainValue) => {
		...
		const additionalVals = (additionalFields[fieldName] || []).reduce(
	...
	const flattenData = (fields, acc = {}) => {
		fields.forEach((field) => {
			if (field.children && field.children.length > 0) {
				...
				acc[field.name] = calculateTotalForField(
					field.name,
					field.value
				);
			}
					...
					{field.children && field.children.length > 0 ? (
						<div className='bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-lg overflow-hidden '>
							<div
								...
								<div className='flex items-center'>
									<div
										className={`w-1 h-6 bg-emerald-600 ${language === "ar" ? "ml-3" : "mr-3"} rounded-full`}></div>
								...
								<div className='flex items-center space-x-2 space-x-reverse'>
									...
									<div
										className={`transform transition-transform duration-300 ${collapsedSections[field.name] ? "rotate-180" : ""}`}>
						...
						<div className='cal-input-bg'>
							<div className='flex items-center justify-between mb-3'>
								<label className='font-semibold text-gray-700 text-sm flex-1 max-[515px]:text-xs'>
									...
									{additionalFields[field.name] &&
										additionalFields[field.name].length >
											0 && (
							...
							<div className='relative mb-3'>
								...
								/>
								<span
									className={`DA  absolute ${language === "ar" ? "right-3" : "left-3"}`}>
									{language === "ar" ? "د.ج" : "DZD"}
								</span>
								<div
							...
							{additionalFields[field.name] &&
								additionalFields[field.name].map(
									(additionalField, idx) => (
										<div
											key={additionalField.id}
											className={`relative mb-3 ${language === "ar" ? "mr-4" : "ml-4"}`}>
											<div className='flex items-center'>
	...
	return (
		<div
			...
			className='w-full mx-auto min-h-screen bg-gradient-to-br from-gray-50 to-emerald-50'>
				...
				<div className='max-w-4xl mx-auto'>
					<div className='bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden'>
						<div className='form-header bg-gradient-to-r from-gray-50 to-emerald-50 border-b max-[515px]:p-4 border-gray-200 p-6'>
							<div className='flex items-center'>
								<div
									className={`w-3 h-8 bg-emerald-600 ${language === "ar" ? "ml-4" : "mr-4"} rounded-full`}></div>
								<div>
									<h2 className='text-xl font-bold text-gray-800 mb-1'>
										{t("ui.assetsData")}
									</h2>
									<p className='text-gray-600 text-sm'>
										{t("ui.assetsDescription")}
									</p>
								</div>
							</div>
						</div>
						...
						<div className='p-8 form-container'>
							<div className='form-cont2 mb-3'>
								<div className='flex items-center mb-4'>
									<div
										className={`w-1 h-6 bg-blue-600 ${language === "ar" ? "ml-3" : "mr-3"} rounded-full`}></div>
									<h3 className='font-bold text-blue-800 text-lg'>
										{t("ui.calculationMethod")}
									</h3>
								</div>
								...
								<div className='relative'>
									<select
										...
										onChange={(e) =>
											setMethodCalcul(e.target.value)
										}
										value={methodCalcul}>
										<option value='Maliki'>
											{t("methods.Maliki")}
									...
									<div
										className={`absolute ${language === "ar" ? "left-4" : "right-4"} top-1/2 transform -translate-y-1/2 pointer-events-none`}>
					...
					<div className='mt-8 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-lg p-6 max-[460px]:p-4'>
						<div className='flex items-center'>
							<div
								className={`bg-amber-100 p-3 rounded-full ${language === "ar" ? "ml-4" : "mr-4"}`}>
