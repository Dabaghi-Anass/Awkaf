...
export const Printed = () => {
  ...
  return (
    <div
      ...
      className='hidden print:block p-8 text-black border border-gray-300 bg-white'
    >
      ...
      <div className='space-y-3 text-[1em]'>
        ...
        <div className='flex justify-between'>
          ...
          <span>{formatNumber(nissab)}</span>
        </div>
        <div className='flex justify-between'>
          <span>الوعاء الزكوي:</span>
          <span>{formatNumber(zakatFormInfos.totalAmount)}</span>
        </div>
        <div className='flex justify-between font-bold text-green-700'>
          <span>مبلغ الزكاة المستحق:</span>
          <span>{formatNumber(zakatFormInfos.zakatAmount)}</span>
        </div>
      </div>
